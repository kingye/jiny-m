...
export class OpenCodeService {
  ...
  private async ensureServerStarted(): Promise<void> {
    ...
    if (this.server && this.client) {
      ...
      logger.warn("OpenCode server is no longer responsive, restarting...", {
        port: this.port,
      });
      try {
        this.server.close();
   ...
   */
  private async ensureThreadOpencodeSetup(
    threadPath: string,
  ): Promise<boolean> {
    ...
    const expectedSmallModel = modelOverride
      ? undefined
      : this.getSmallModelString();
    ...
    try {
      ...
      if (await existing.exists()) {
        ...
        const existingCommand = JSON.stringify(
          content?.mcp?.["jiny_reply"]?.command,
        );
        const expectedCommand = JSON.stringify(toolCommand);
        if (
          existingCommand === expectedCommand &&
          content?.mcp?.["jiny_reply"]?.environment?.JINY_ROOT ===
            process.cwd() &&
  ...
  private async findFreePort(): Promise<number> {
    ...
    logger.debug("Searching for free port", {
    ...
    for (let port = startPort; port < endPort; port++) {
      ...
      if (inUse) {
        logger.debug("Port in use (active listener detected), skipping", {
          port,
        });
        continue;
      }
    ...
    throw new Error(
      `No free ports available in range ${startPort}-${endPort - 1}`,
    );
  }
  ...
  async generateReply(
    ...
    const configFreshlyWritten =
      await this.ensureThreadOpencodeSetup(threadPath);
    ...
    if (configFreshlyWritten) {
      logger.info(
        "OpenCode config changed, restarting server and creating new session",
        { threadPath },
      );
    ...
    aiReply.replySentByTool =
      replySentByTool || this.checkToolUsed(resultParts as Array<any>);
    ...
    if (aiReply.replySentByTool) {
      ...
      if (!signalConfirmed) {
        logger.error(
          "Reply tool reported success via SSE but signal file not found — stale session detected",
          {
            sessionId: session.sessionId,
        ...
        await this.cleanupStaleSignalFile(threadPath);
        const retryResult = await this.promptWithProgress(
          newSession,
          threadPath,
          systemPrompt,
          prompt,
        ...
        retryReply.replySentByTool =
          retryResult.replySentByTool ||
          this.checkToolUsed(retryResult.parts as Array<any>);
  ...
  private async createNewSession(threadPath: string): Promise<ThreadSession> {
    ...
    logger.info("Created new session (replacing old one)", {
      sessionId: newSession.sessionId,
      thread: threadName,
      ...
      if (await file.exists()) {
        ...
        try {
          ...
          if (!result || !result.data) {
            logger.warn("Session no longer exists, creating new one", {
              sessionId: session.sessionId,
            });
          ...
          if ((error as any)?.code === "NOT_FOUND") {
            logger.warn("Session NOT_FOUND, creating new one", {
              sessionId: session.sessionId,
            });
            return this.createNewSession(threadPath);
          }
  ...
  private async promptWithProgress(
    ...
    const ACTIVITY_TIMEOUT = 600_000; // 10 minutes of silence → timeout
    const TOOL_ACTIVITY_TIMEOUT = 1200_000; // 20 minutes when a tool is actively running
    ...
    try {
      ...
      logger.warn(
        "SSE subscription failed, falling back to blocking prompt()",
        {
          error: error instanceof Error ? error.message : "Unknown",
        },
      );
    ...
    try {
      ...
      logger.info(`Opencode will run in ${modeOverride ?? "build"} mode`);
    } catch (error) {
    ...
    logger.info("Prompt sent (async), waiting for events...", {
      sessionId,
    ...
    const activityCheckInterval = setInterval(() => {
      ...
      const activeTimeout = lastToolName
        ? TOOL_ACTIVITY_TIMEOUT
        : ACTIVITY_TIMEOUT;
      if (silenceMs > activeTimeout) {
        logger.warn(
          `Activity timeout: no events for ${Math.round(activeTimeout / 1000)}s`,
          {
            elapsed: `${Math.round(elapsedMs / 1000)}s`,
            parts: accumulatedParts.length,
        ...
        sessionError = new Error(
          `No activity from OpenCode for ${Math.round(activeTimeout / 1000)} seconds`,
        );
        // Force-close the SSE stream so the for-await loop unblocks immediately
        if (sseStream) {
          try {
            sseStream.return(undefined);
          } catch {
            /* ignore */
          }
        }
    ...
    try {
      for await (const event of sseStream) {
        ...
        const eventSessionId =
          properties.sessionID || properties.part?.sessionID;
        ...
        switch (eventType) {
          case "message.part.updated": {
            ...
            const existingIdx = accumulatedParts.findIndex(
              (p: any) => p.id === part.id,
            );
            if (existingIdx >= 0) {
            ...
            if (part.type === "tool" && part.tool) {
              ...
              const isReplyTool =
                part.tool.includes("reply_email") ||
                part.tool.includes("reply_message");
              ...
              if (prevStatus !== toolStatus) {
                ...
                if (
                  ...
                  const details = this.summarizeToolInput(
                    part.tool,
                ...
                if (
                  ...
                  logger.info("Reply tool input from AI", {
                    ...
                    contextLength:
                      typeof contextVal === "string" ? contextVal.length : 0,
              ...
              if (isReplyTool) {
                if (toolStatus === "completed") {
                  ...
                  if (isErrorOutput) {
                    ...
                    logger.info(
                      "Reply MCP tool completed successfully (detected via SSE)",
                      {
                        tool: part.tool,
                      },
                    );
          ...
          case "session.error": {
            ...
            if (error?.name === "ContextOverflowError") {
              logger.warn(
                "ContextOverflowError via SSE, will retry with new session",
              );
  ...
  private async promptBlocking(
      ...
      result = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error("OpenCode prompt timed out (blocking fallback)"),
              ),
            PROMPT_TIMEOUT,
          ),
        ),
    ...
    if (!result || !result.data) {
      throw new Error(
        "Failed to get response from OpenCode (blocking fallback)",
      );
    }
  ...
  private checkToolUsed(parts: Array<Record<string, any>>): boolean {
    ...
    for (const part of parts) {
      ...
      const toolId = (part.tool || part.toolName || part.name || "")
        .toString()
        .toLowerCase();
      ...
      if (
        toolId.includes("reply_email") ||
        toolId.includes("reply_message") ||
        (partType === "tool" && toolId.includes("reply"))
      ) {
        ...
        logger.warn(
          "reply_email tool call detected but state indicates failure",
          { status: partStatus },
        );
      }
    }
  ...
  private async cleanupStaleSignalFile(threadPath: string): Promise<void> {
    ...
    try {
      ...
      logger.info("Cleaned up stale signal file from previous run", {
        signalFile,
      });
    } catch {
  ...
  private getReplyToolCommand(): { toolCommand: string[] } {
    ...
    if (mainBinary) {
      ...
      if (existsSync(replyToolBinary)) {
        logger.debug("Reply tool: using compiled binary", {
          path: replyToolBinary,
        });
    ...
    const commonPaths = [
      "/usr/local/bin/jiny-m-reply-tool",
      "/usr/bin/jiny-m-reply-tool",
    ];
  ...
  private summarizeToolInput(
    ...
    if (
      toolName.includes("reply_email") ||
      toolName.includes("reply_message")
    ) {
      return null;
    }
    ...
    if (toolName === "todowrite" || toolName.endsWith("_todowrite")) {
      ...
      if (Array.isArray(todos)) {
        const summary = todos.map(
          (t: any) => `[${t.status}] ${str(t.content, 80)}`,
        );
