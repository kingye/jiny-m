...
export class PromptBuilder {
  ...
  async buildSystemPrompt(threadPath: string): Promise<string> {
    ...
    parts.push(
      `Your working directory is "${threadPath}". You MUST only read, write, and access files within this directory. Do NOT access files outside this directory.`,
    );
    parts.push("");
    ...
    parts.push("CRITICAL: Always use jiny_reply_reply_message tool.");
    parts.push("");
    ...
    try {
      ...
      if (threadSystemPrompt.trim()) {
        logger.info("Loaded thread system.md", {
          path: systemMdPath,
          length: threadSystemPrompt.trim().length,
        });
        parts.push("");
        parts.push(threadSystemPrompt.trim());
      }
    } catch (err) {
      logger.debug("No system.md found", {
        threadPath,
        ...
        try {
          ...
          let fileContent =
            content.length > MAX_PER_FILE
              ? truncateText(content, MAX_PER_FILE)
              : content;
